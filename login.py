...
from PIL import ImageTk #help to import images(if png image then not required)
def login():
    if usernameEntry.get()=='' or passwordEntry.get()=='':
       messagebox.showerror('Error','Fields cannot be empty') 
    elif usernameEntry.get()=='User' and passwordEntry.get()=='1234':
        messagebox.showinfo('Successs','Welcome') 
        window.destroy()
        import sms 
...
window.resizable(False,False) #can't change window size now
