...
def delete_student():
    ...
    con.commit()
    messagebox.showinfo('Default',f'The id- {content_id} is deleted successfully')
    query ='select *from student'
    mycursor.execute(query)
    fetched_data=mycursor.fetchall()
    studentTable.delete(*studentTable.get_children())
...
def add_data():
    if idEntry.get()=='' or nameEntry.get()=='' or phoneEntry.get()=='' or emailEntry.get()=='' or addressEntry.get()=='' or genderEntry.get()=='' or dobEntry.get()=='' :
        messagebox.showerror('All fields are required',parent=screen)
    else:
...
root.title('Student Management System') # s[0]
...
showstudentButton=ttk.Button(leftFrame,text='Show Students',width=25,state=DISABLED,command=toplevel_data)
showstudentButton.grid(row=5,column=0,pady=20)

exitstudentButton=ttk.Button(leftFrame,text='Exit Students',width=25,command=iexit)
...
studentTable.column('Gender',width=400,anchor=CENTER)
